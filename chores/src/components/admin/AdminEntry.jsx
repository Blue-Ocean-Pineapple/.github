 ...
 let btnIndex = 1;
  ...
  let buttonStatus = ["awaiting", "approved", "in-progress", "complete"];
    ...
    const status = (e) => {
      ...
      axios.put(`http://localhost:3001/api/staff/updateTicketStatus`, {clientStatus: e.target.value})
      .then((response) => {
        console.log('hi')
        setIndex(btnIndex += 1);
  ...
  <ChakraProvider>
          ...
          <Td>{bool()}</Td>
          <Td><Button value={buttonStatus[index]} onClick={(e) =>status(e) }>{buttonStatus[index]}</Button></Td>
